  ...
  BEGIN
    ...
    SELECT
      @x = @x + cast((SELECT
                        [VU_BU-Name]
                      FROM [BVK$Contact]
                      WHERE [No_] = [Relation to No_]) AS NVARCHAR(50)) + N','
    FROM dbo.[BVK$Address Relations]
    WHERE [Contact No_] = @contactno
    IF len(@x) > 0
      SELECT
        @x = left(@x, len(@x) - 1)
...
--when 'bvk' then c.[MyBVKLogin] + 'bvk' + c.[MyBVKLogin]
    ...
    [Country Code] = CASE c.[Country_Region Code]
    WHEN '' THEN N'D'
    ELSE c.[Country_Region Code]
    END,
    [Phone No_] = dbo.bvkplatform_isnull(c.[Phone No_]),
    [Fax No_] = dbo.bvkplatform_isnull(c.[Fax No_]),
    [Mobile Phone No_] = dbo.bvkplatform_isnull(c.[Mobile Phone No_]),
    [Phone No_ privat] = dbo.bvkplatform_isnull(c.[Phone No_ private]),
    [Fax No_ privat] = dbo.bvkplatform_isnull(c.[Fax No_ private]),
    ...
    [Function Codes] = dbo.bvkplatform_isnull(dbo.bvkplatform_getfunctions2(c.[No_])),
    [Ehrenamt] = replace(
        replace(
            replace(dbo.bvkplatform_isnull(dbo.bvkplatform_getfunctions(c.[No_])),
                    'Bezirksverband ', 'BV - '),
            'Regionalverband ', 'RV - '),
        ', ', ','),
    [Companies] = dbo.bvkplatform_isnull(dbo.bvkplatform_getcompanies(c.[No_])),
    [Ausbildung] = dbo.bvkplatform_isnull(dbo.bvkplatform_geteducation(c.[No_])),
